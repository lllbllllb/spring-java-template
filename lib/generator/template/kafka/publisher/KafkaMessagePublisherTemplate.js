...
public class ${className} implements ${parentClassName} {
    ...
    public void publish(${['String key', payloadClassName + ' payload', parameters.map(paramName => 'String ' + paramName).join(', ')].join(', ')}) {
        var parameters = new HashMap<String, String>();
        ${!parameters ? '\r' : parameters.map(parameter => `parameters.put("${parameter}", ${parameter});`).join('\n')}
